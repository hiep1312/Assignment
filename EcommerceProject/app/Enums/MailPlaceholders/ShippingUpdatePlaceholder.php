...
enum ShippingUpdatePlaceholder: string
{
    ...
    protected static function resolveValues(Order $order, Message $message): array
    {
        return [
            ...
            self::CUSTOMER_AVATAR->value => function() use ($order, $message){
                ...
                return <<<HTML
                    <div style="flex-shrink: 0;">
                        <img src="{$avatarCid}" alt="Customer avatar" style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover; border: 1px solid #667eea;">
                    </div>
                HTML;
            },
            ...
            self::STATUS->value => fn() => ucwords(str_replace('_', ' ', OrderStatus::tryFrom($order->status)?->case ?? '')),
            self::ESTIMATED_DELIVERY->value => fn() => $order->created_at->addDays(5)->format('m/d/Y H:i A')
        ];
    }

    public static function replacePlaceholders(string $template, Order $order, Message $message, ?array $variable = null): string
    {
        $order->loadMissing('user', 'shipping', 'items.productVariant.product');
